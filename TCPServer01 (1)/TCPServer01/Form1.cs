            ...
            try
            {
                lock(mlClientSocks)
                {
                    ...
                    lock (mlClientSocks)
                    {
                        ...
                        ClientNode cn1 = null;
                        //cn1 = mlClientSocks.Find(x => x.strId == lbClients.SelectedItem.ToString());
                        cn1 = mlClientSocks.Find(x => x.strId == lbClients.Items[0].ToString());
                        cn1.Tx = new byte[8];
                        ...
                        try
                        {
                            if (cn1 != null)
                            {
                                if (cn1.tclient != null)
                                {
                                    if (cn1.tclient.Client.Connected)
                                    {
                                        cn1.Tx = Encoding.ASCII.GetBytes(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:fff") + "\r\n");
                                        cn1.tclient.GetStream().BeginWrite(cn1.Tx, 0, cn1.Tx.Length, onCompleteWriteToClientStream, cn1.tclient);
                                    }
                                }
                            }
                        }
